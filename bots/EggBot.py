...
class EggBotConnection:
    ...
    def bikeUntilEgg(self) -> None:
        ...
        self.commands.poke(self.pointers['player_x'], "0x00000CC400000040AC3F1E44000000000000008000000080000000800000803F0000803F0000803F0000803F") # Please don't look at them I'm ashamed

        while self.commands.peek(self.pointers['is_egg'], 1) != b'01\n': 
            if down:
                self.commands.setStick("LEFT", "0x0", "-0x8000")
            else:
                self.commands.setStick("LEFT", "0x0", "0x7FFF")
    ...
    def collectEgg(self) -> None:
        self.commands.poke(self.pointers['player_x'], "0xFCFF0AC400000040D6FE21440000000000000080F404353F00000080F404353F0000803F0000803F0000803F") # Please god send help it works
        time.sleep(0.4)
        self.commands.clickA(0.5)
    ...
    def determineShiny(self, mon: bytearray):
        pid = int(mon[31:27:-1].hex(),16)

        tid = int(mon[13:11:-1].hex(),16)

        sid = int(mon[15:13:-1].hex(),16)
    ...
    def checkEgg(self) -> bool:
        ...
        if "0x" + DecryptEgg.hex().upper()[:12] == "0x000000000000":
            return False
        print("0x" + DecryptEgg.hex().upper()[:12])
        return self.determineShiny(DecryptEgg)


    def mainRoutine(self) -> None:
        encounters = 0
        while True:
            print(f"Starting Egg {encounters+1}")
            self.bikeUntilEgg()
            self.collectEgg()
            if self.checkEgg():
                break
