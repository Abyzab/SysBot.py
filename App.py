...
def makeBlankConfig():
    defaultConfig = {"token": "", "tradeTimeout": 60, "maxQueue": 20, "botOwner": [], "address" : "", "port" : 6000, "botType" : "Sysbot"}
...
if __name__ == "__main__":
    ...
    if os.path.exists(f"{os.path.abspath(os.curdir)}/config.json"):
        ...
        if Configs["botType"] == "EggBot":
            EggBot = EggBotConnection(Configs["address"], Configs["port"])
            EggBot.mainRoutine()
            ...
            SysBot = SysBotConnection(Configs["address"], Configs["port"], startTradeRoutine, q, Configs, settings, PyBot)
